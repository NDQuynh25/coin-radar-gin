...
func errorResponse(err error) dto.Response {
	...
	if errors.As(err, &ve) {
		...
		return dto.ValidationError("validation_failed", "one or more fields are invalid", fields)
	}
	...
	if errors.Is(err, io.EOF) {
		return dto.Error("invalid_request", "request body is empty")
	}

	// Malformed JSON or anything else — don't leak internal parser details.
	return dto.Error("invalid_request", "malformed JSON body")
}
