...
func (s *Service) Login(ctx context.Context, email, password string) (*user.User, TokenPair, error) {
	...
	if err != nil { if errors.Is(err, user.ErrNotFound) { return nil, TokenPair{}, ErrInvalidCredentials }; return nil, TokenPair{}, err }
	if u.PasswordHash == "" || bcrypt.CompareHashAndPassword([]byte(u.PasswordHash), []byte(password)) != nil { return nil, TokenPair{}, ErrInvalidCredentials }
	pair, err := s.generateTokens(u.ID, s.now())
	if err != nil { return nil, TokenPair{}, err }
	return u, pair, nil
}

func (s *Service) TelegramLogin(ctx context.Context, data TelegramAuthData) (*user.User, TokenPair, error) {
	now := s.now()
	if err := s.telegram.verify(data, now); err != nil { return nil, TokenPair{}, err }
	u, err := s.users.FindByTelegramID(ctx, data.ID)
	switch {
	case err == nil:
		if data.Username != "" && data.Username != u.Username { u.Username = data.Username; u.UpdatedAt = now; if err := s.users.Update(ctx, u); err != nil { return nil, TokenPair{}, err } }
	case errors.Is(err, user.ErrNotFound):
		u = &user.User{TelegramID: data.ID, Username: data.Username, CreatedAt: now, UpdatedAt: now}
		if err := s.users.Create(ctx, u); err != nil { return nil, TokenPair{}, err }
	default: return nil, TokenPair{}, err
	}
	pair, err := s.generateTokens(u.ID, now)
	if err != nil { return nil, TokenPair{}, err }
	return u, pair, nil
...
func (s *Service) Refresh(ctx context.Context, refreshToken string) (TokenPair, error) {
	...
	if _, err := s.users.FindByID(ctx, userID); err != nil { return TokenPair{}, ErrInvalidToken }
	return s.generateTokens(userID, s.now())
}

func (s *Service) GetUser(ctx context.Context, id int64) (*user.User, error) { return s.users.FindByID(ctx, id) }

func (s *Service) VerifyAccessToken(accessToken string) (int64, error) { return s.verifyToken(accessToken, accessTokenType) }
